...
boost::mutex vectorLock;
vector<string> hasUsed;
...
void generateSymbol(xml_document<> &response,xml_node<> *results ){
    ...
    string id=generateValue(random()%10);
    vectorLock.lock();
    hasUsed.push_back(id);
    vectorLock.unlock();
    attrValues = {id};
...
void gennerateQuery(xml_document<> &response,xml_node<> *results){
    vector<string> attrNames = {"id"};
    vectorLock.lock();
    int size = hasUsed.size();
    if(size!=0) {
        string id=hasUsed[size-1];
        vector<string> attrValues = {id};
...
string generateTransaction(){
    ...
    char *attr_name = response.allocate_string("id");
    char *attr_value = response.allocate_string(generateValue(random()%10).c_str());
    xml_attribute<> *newAttr = response.allocate_attribute(attr_name, attr_value);
